...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::WSK)
    {
    }
...
//    ui->label_wastake4->setVisible(showImmature || showWatchOnlyImmature);
...
//        model->getWatchBalance(), model->getWatchUnconfirmedBalance(), model->getWatchImmatureBalance());
        ...
        connect(ui->blabel_wastake, SIGNAL(clicked()), this, SLOT(openMyAddresses()));

        emit model->makeBalance();
    }

    // update the display unit, to not use the default ("wastake")
    updateDisplayUnit();
}
...
void OverviewPage::updateMasternodeInfo()
{
  ...
  if (masternodeSync.IsBlockchainSynced() && masternodeSync.IsSynced())
  {
   ...
   std::vector<CMasternode> vMasternodes = mnodeman.GetFullMasternodeMap();
    ...
    int maxMnValue = std::max( { mn1, mn2 }, [](const int& s1, const int& s2) { return s1 < s2; });
    ...
    CAmount tNodesSumm = mn1*1000 + mn2*3000;
  ...
  if (CurrentBlock >= 0) {
      ui->label_lcolat->setText("1000 WSK");
      ui->label_mcolat->setText("3000 WSK");
  }
...
void OverviewPage::updateBlockChainInfo()
{
    if (masternodeSync.IsBlockchainSynced())
    {
        ...
        double BlockReward = GetBlockValue(CurrentBlock);
        double BlockRewardwastake =  static_cast<double>(BlockReward/COIN);
        ...
        ui->label_CurrentBlockReward_value->setText(QString::number(BlockRewardwastake, 'f', 1).append(" | ") + QString::number(GetSporkValue(SPORK_11_DEV_FEE)).append("%"));

        ui->label_Supply_value->setText(QString::number(chainActive.Tip()->nMoneySupply / COIN).append(" WSK"));
...
void OverviewPage::on_pushButton_Website_clicked() {
    QDesktopServices::openUrl(QUrl("https://wastake.com/", QUrl::TolerantMode));
}
void OverviewPage::on_pushButton_Discord_clicked() {
    QDesktopServices::openUrl(QUrl("https://discordapp.com/", QUrl::TolerantMode));
}
void OverviewPage::on_pushButton_Telegram_clicked() {
    QDesktopServices::openUrl(QUrl("https://t.me/wastake/", QUrl::TolerantMode));
}
void OverviewPage::on_pushButton_Twitter_clicked() {
    QDesktopServices::openUrl(QUrl("https://twitter.com/wastake/", QUrl::TolerantMode));
}
void OverviewPage::on_pushButton_Reddit_clicked() {
    QDesktopServices::openUrl(QUrl("https://bitcointalk.org/", QUrl::TolerantMode));
}
void OverviewPage::on_pushButton_Medium_clicked() {
    QDesktopServices::openUrl(QUrl("https://github.com/wastake/", QUrl::TolerantMode));
}
/*
...
void OverviewPage::on_pushButton_Explorer_clicked() {
    QDesktopServices::openUrl(QUrl("http://165.22.200.154:3000", QUrl::TolerantMode));
}
